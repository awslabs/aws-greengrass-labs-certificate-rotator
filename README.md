...
The **aws.greengrass.labs.CertificateRotator** component supports just two configuration parameters: **keyAlgorithm** and **signingAlgorithm**. The component [validates configuration updates](https://docs.aws.amazon.com/greengrass/v2/developerguide/ipc-component-configuration.html#ipc-operation-subscribetovalidateconfigurationupdates). If there is an attempt to merge invalid settings, the deployment will fail. The encryption family (RSA or ECC) of the signing algorithm has to match that of the key algorithm.

#### keyAlgorithm

This sets the algorithm of the new private key generated by the component when a rotation is performed. The supported key algorithms are:
...
These match the keys types that can be used with the [CreateCertificateFromCsr](https://docs.aws.amazon.com/iot/latest/apireference/API_CreateCertificateFromCsr.html) operation.
...
The default signing algorithm is **SHA256WITHRSA** as seen in [recipe.yaml](recipe.yaml). To change the signing algorithm, the component configuration to be merged has the following form:
